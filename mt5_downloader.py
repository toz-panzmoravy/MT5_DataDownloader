...
# Počet dní zpět pro každý timeframe
TIMEFRAME_DAYS = {
    "M1": 30,   # 30 dní pro M1
    "M3": 60,   # 60 dní pro M3
    "M5": 365,  # 365 dní pro M5
    "M10": 365, # 365 dní pro M10
    "M15": 365  # 365 dní pro M15
}
DAYS_BACK = 365  # Výchozí počet dní (pro zpětnou kompatibilitu)
...
def download_data(symbol, timeframe, timeframe_name, days_back=365):
    ...
    if timeframe == mt5.TIMEFRAME_M1:
        # Pro M1: 60 svíček za hodinu, max 100000 svíček (30 dní = ~43,200 svíček)
        count = min(days_back * 24 * 60, 100000)
        rates = mt5.copy_rates_range(symbol, timeframe, from_date, to_date)
        if rates is None or len(rates) == 0:
            rates = mt5.copy_rates_from(symbol, timeframe, to_date, count)
        if rates is None or len(rates) == 0:
            rates = mt5.copy_rates_from(symbol, timeframe, from_date, count)
    elif timeframe == mt5.TIMEFRAME_M3:
        # Pro M3: 20 svíček za hodinu, max 100000 svíček (60 dní = ~28,800 svíček)
        count = min(days_back * 24 * 20, 100000)
        rates = mt5.copy_rates_range(symbol, timeframe, from_date, to_date)
        if rates is None or len(rates) == 0:
            rates = mt5.copy_rates_from(symbol, timeframe, to_date, count)
        if rates is None or len(rates) == 0:
...
def main():
    ...
    for instrument in INSTRUMENTS:
        for timeframe_name, timeframe in TIMEFRAMES.items():
            try:
                # Použij specifický počet dní pro každý timeframe
                days_back = TIMEFRAME_DAYS.get(timeframe_name, DAYS_BACK)
